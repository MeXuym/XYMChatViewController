...
-(void)backspaceTextView
{
    ...
    if([allText length]>0)
    {
        int textCursorPos=(int)textView_.selectedRange.location;
        
        //防护输入法未完成的情况
        if(textCursorPos>[allText length])
        {
            textCursorPos=(int)[allText length];
        }
...
-(void)startBackspace
{
    ...
    if(!scheduledTimer_)
    {
        scheduledTimer_=[NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(backspaceTextView) userInfo:nil repeats:YES];
    }
}
